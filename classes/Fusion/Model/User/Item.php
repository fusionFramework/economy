 ...
 */
class Fusion_Model_User_Item extends ORM
{
	 ...
	 * if successful it will return a user item instance of where the item copies transferred to.
	 *
	...
	public function transfer(Model_User $user, $amount = 1)
	{
		...
		{
			$this->_relocate($user->id, 'inventory', $amount, !$this->item->unique);
			return Fusion::$log->create('transfer' . $this->item->id, 'items.gift', ':item_name transferred to :other_user',
