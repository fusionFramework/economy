 ...
 */
class Controller_Shop extends Controller_Fusion_Site {
	...
	public function action_index()
	{
		...
		$config = Kohana::$config->load('items.shop');
	...
	public function action_upgrade()
	{
		...
		$config = Kohana::$config->load('items.shop.size');
	...
	public function action_stock()
	{
		...
		$pagination = Paginate::factory($items, ['total_items' => $config['stock']['pagination']])->execute();
	...
	public function action_logs()
	{
		...
		$logs = ORM::factory('Log')
			...
			->where('time', '>', strtotime('-'.Kohana::$config->load('items.shop.log_retention')))
			->limit(Kohana::$config->load('items.shop.log_limit'))
			->order_by('id', 'DESC')
			->find_all();
	...
	public function action_buy()
	{
		...
		if ($this->request->param('item_id', false) != false)
		{
			...
			else
			{
				...
				//send notification
				if(Kohana::$config->load('items.shop.notify_sale') == true)
				{
