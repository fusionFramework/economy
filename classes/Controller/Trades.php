 ...
 */
class Controller_Trades extends Abstract_Controller_Frontend {
		...
		else
		{
			...
			if(count($bids) > 0)
			{
				foreach($bids as $bid) {
					...
					$log = Journal::log('item.trade.' . $id . '.reject', 'item', 'Bid from :user declined', array(':user' => $this->user->username));
